...
//Bring in the FS package. 11/20/2020
const fs = require("fs");
// Object to hold all submission history for /sayHello
let history = {
    submissions: []    //submission property
};
...
if (fs.existsSync("history.json")) {
    ...
    // If it doesn't exist,  convert history into JSON and save it to history.json
...
app.post("/sayHello", (request, response) => {
    ...
    let historyEntry = {   //creating this object
        number: userNumberChoice,  //a property
        winningNumber: winningNumber,  //calling a variable
        timestamp: Date.now()      //a property
    };

    //place entry in our history object
    history.submissions.push(historyEntry);   //push ==> adding into    submissions: []
